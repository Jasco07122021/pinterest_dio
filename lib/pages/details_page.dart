...
class _DetailsPinterestState extends State<DetailsPinterest> {
  ...
  _checkLoadData() {
    DioServer.GET(DioServer.API_SEARCH_COLLECTIONS,
            DioServer.paramsSearch(query: widget.obj!.title))
        .then((value) {
      if (value != null) _checkLoadPage(value);
    });
  }
  ...
  _loadDateDio() async {
    DioServer.GET(
            DioServer.API_SEARCH_COLLECTIONS,
            DioServer.paramsSearch(
                query: obj.title, page: 1 + Random().nextInt(totalPage)))
        .then((value) {
      if (value != null) _showDataDio(value);
    });
  }

  _showDataDio(String response) {
    Map<String, dynamic> map = jsonDecode(response);
    list.addAll(DioServer.parseCollectionResponse(jsonEncode(map['results'])));
    setState(() {
      isLoading = false;
  ...
  loadMore() {
    ...
    DioServer.GET(
        DioServer.API_SEARCH_COLLECTIONS,
        DioServer.paramsLoadMore(
          page: 1 + Random().nextInt(totalPage),
          query: obj.title,
        )).then((value) {
      if (value != null) _showDataDio(value);
    });
