...
class _HomePageState extends State<HomePage> with RouteAware {
  ...
  _loadDateDio() {
    DioServer.GET(DioServer.API_COLLECTIONS,
            DioServer.paramsSearch(page: 1 + Random().nextInt(totalPage)))
        .then((value) {
  ...
  _showDataDio(String response) async {
    setState(() {
      list.addAll(DioServer.parseCollectionResponse(response));
      isLoading = false;
  ...
  loadMoreDio() async {
    ...
    DioServer.GET(DioServer.API_COLLECTIONS,
            DioServer.paramsLoadMore(page: 1 + Random().nextInt(totalPage)))
        .then((value) {
      if (value != null) _showDataDio(value);
    });
  }
